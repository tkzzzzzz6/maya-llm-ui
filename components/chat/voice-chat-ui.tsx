...
import { useVoiceHandler } from "./chat-hooks/use-voice-handler"
...
export const VoiceChatUI: FC<VoiceChatUIProps> = ({}) => {
  ...
  const [lastMessageRole, setLastMessageRole] = useState<"user" | "assistant">(
    "assistant"
  )
  ...
  const voiceHandler = useVoiceHandler({
    onTranscriptionComplete: async (text: string) => {
      // 语音转文字完成后,自动发送消息
      setUserInput(text)
      await handleSendMessage(text, chatMessages, false)
  ...
  useEffect(() => {
    ...
    // 如果最后一条消息是 assistant 的回复,且不是我们上次播放的
    if (
      ...
      // 自动播放 TTS
      voiceHandler.playTextToSpeech(messageText, {
        voice: "alloy", // 可以从设置中获取
        model: "tts-1"
      })

      setLastMessageRole("assistant")
    }
  }, [chatMessages, lastMessageRole])
  ...
  const fetchMessages = async () => {
    ...
    if (fetchedChatMessages.length > 0) {
      const lastMsg =
        fetchedChatMessages[fetchedChatMessages.length - 1].message
      setLastMessageRole(lastMsg.role as "user" | "assistant")
    }
  }
